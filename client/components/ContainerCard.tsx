...
const ContainerCard: React.FC<ContainerCardProps> = ({
    ...
    const [internalMessage, setInternalMessage] = React.useState<{ text: string; type: 'success' | 'error' } | null>(null);
    ...
    const handleBooking = async (containerId: string) => {
        ...
        try {
            ...
            if (response.success) {
                ...
                setMessage({ text: 'Failed to book the container. Please try again.', type: 'error' });
            }
    ...
    const confirmContainerReadiness = async () => {
        try {
            ...
            if (res.data.success) {
                setMessage({ type: 'success', text: 'Container marked as ready for shipment!' });
                onBookingSuccess();
            } else {
    ...
    return (
        <>
            <div
                className="relative overflow-hidden rounded-xl shadow-lg bg-white border border-slate-200 transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 group cursor-pointer"
            >
                <div
                    className={`absolute top-0 right-0 px-3 py-1 text-xs font-bold text-white ${statusColors[container.status as keyof typeof statusColors] || 'bg-amber-500'
                        }`}
                >
                ...
                <div className="p-6" onClick={() => setIsModalOpen(true)}>
                    <div className="flex items-center mb-4">
                        ...
                        <h3 className="text-xl font-bold text-slate-800">{container.containerId}</h3>
                    </div>
                    ...
                    <div className="space-y-3">
                        ...
                        {container.goods && container.goods.length > 0 && (
                            <div className="flex items-center">
                                ...
                                <span className="text-sm font-medium text-slate-600">
                                    Contains {container.goods.length} item{container.goods.length !== 1 ? 's' : ''}
                                </span>
                            </div>
                        )}
                    </div>
                    ...
                    {container.status === 'AVAILABLE' && (
                        <div className="mt-6 pt-4 border-t border-slate-100">
                            <button
                                ...
                                className={`w-full flex justify-center items-center gap-2 py-3 px-4 rounded-lg font-semibold text-white bg-gradient-to-r from-cyan-600 to-blue-700 hover:from-cyan-700 hover:to-blue-800 transition-all duration-200 shadow-md ${loading === container.id ? 'opacity-80 cursor-not-allowed' : ''
                                    }`}
                            >
